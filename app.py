...
with st.form("add_form", clear_on_submit=True):
    activity = st.text_input("Activity or Food", placeholder="e.g. Apple, Running")
    calories = st.number_input("Calories (+ for food, - for exercise)", step=10)
    submitted = st.form_submit_button("Add Entry")

    if submitted and activity:
        with get_db() as conn:
            conn.execute('INSERT INTO logs (activity, calories) VALUES (?, ?)', (activity, int(calories)))
...
st.metric("Net Calories", f"{total_calories} kcal")
...
if logs:
    for log_id, act, cal in logs:
        col1, col2 = st.columns([4, 1])
        color = "🔴" if cal > 0 else "🟢"
        col1.write(f"{color} **{act}**: {cal:+d} kcal")
        if col2.button("Delete", key=f"del_{log_id}"):
            with get_db() as conn:
                conn.execute('DELETE FROM logs WHERE id = ?', (log_id,))
            st.rerun()
else:
    st.info("No logs saved yet. Add one above!")
